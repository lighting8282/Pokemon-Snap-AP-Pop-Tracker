...
pokemon_snap_location = {}
pokemon_snap_location.__index = pokemon_snap_location
...
---@type table<string, pokemon_snap_new_return>
NAMED_LOCATIONS = {}
local stale = true
...
---@field name string
---@field side string?
---@field exits table<integer, {[1]:pokemon_snap_new_return, [2]:fun(): accessibilityLevel}>
---@field keys integer
...
---@param name string
---@return pokemon_snap_new_return
function pokemon_snap_location.new(name)
	if name == nil then
		error("pokemon_snap_location name cannot be nil")
	end
	if NAMED_LOCATIONS[name] then
		print("pokemon_snap_location " .. name .. " already exists")
	end
    local self = setmetatable({}, pokemon_snap_location)
	self.name = name
...
---@param exit string|pokemon_snap_new_return pokemon_snap_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function pokemon_snap_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = pokemon_snap_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
---@param exit string|pokemon_snap_new_return pokemon_snap_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function pokemon_snap_location:connect_two_ways(exit, rule)
...
---@param exit string|pokemon_snap_new_return pokemon_snap_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function pokemon_snap_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
---@param exit string|pokemon_snap_new_return pokemon_snap_new_return or code/name
---@param rule? fun(): accessibilityLevel|boolean
function pokemon_snap_location:connect_two_ways_entrance(name, exit, rule)
...
function pokemon_snap_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
---@param rule1? fun(): accessibilityLevel|boolean
---@param rule2? fun(): accessibilityLevel|boolean
function pokemon_snap_location:connect_two_ways_stuck(exit, rule1, rule2)
...
---@return accessibilityLevel
function pokemon_snap_location:accessibility()
...
---@param keys integer
function pokemon_snap_location:discover(accessibility, keys)
...
Entry_Point = pokemon_snap_location.new("Entry_Point")
